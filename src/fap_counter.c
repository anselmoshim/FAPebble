...
int num_drinks = NUM_DRINKS_DEFAULT;

static void update_text() {
  static char body_text[50];
  snprintf(body_text, sizeof(body_text), "%u Faps", num_drinks);
...
static void window_load(Window *me) {
  ...
  text_layer_set_text(header_text_layer, "Fap Counter");
  ...
  if (num_drinks >= 1)
  {
    text_layer_set_text(label_text_layer, "Good Start!");
    update_text();
    return;
  }
  if (num_drinks >= 10)
  {
    text_layer_set_text(label_text_layer, "Do More!");
    update_text();
    return;
  }
  if (num_drinks >= 25)
  {
    text_layer_set_text(label_text_layer, "What a Boss!");
    update_text();
    return;
  }
  */
  text_layer_set_text(label_text_layer, "Lol Wut!?");
